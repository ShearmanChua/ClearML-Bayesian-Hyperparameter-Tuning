...
class BOtask:
	def __init__(self, base_task_project_name, base_task_name, params, metric, metric_sign, hyperparameters_section, objective_metric_title, objective_metric_series, tuner_project_name='hyperparameter_tuning', tuner_task_name='hyperparameter tuning', optimizer='BayesianOptimization', best_epoch=False, max_number_of_concurrent_tasks=1,execution_queue='default',time_limit_per_job=10., pool_period_min=0.2, total_max_jobs=10, min_iteration_per_job=3,max_iteration_per_job=6):
	...
	def job_complete_callback(
		...
		try:
			...
			for i in range(1, len(objectives)):
				if(task_object.metric_sign == 'min'):
					if(objectives[i]<objective):
						objective = objectives[i]
						epoch = i
				elif(task_object.metric_sign == 'max'):
					if(objectives[i]>objective):
						objective = objectives[i]
						epoch = i
	...
	def BO_tuner(self):
		...
		if self.metric_sign == 'max':
			print("Optimizing model using maximizing objective")
			objective = self.acc_objective
		elif self.metric_sign == 'min':
			print("Optimizing model using minimizing objective")
	...
	def acc_objective(self, **data):
		...
		if(cloned_task.get_last_scalar_metrics()):
			...
			self.logger.report_scalar(title=self.objective_metric_title,series=self.objective_metric_series, value=cloned_task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last'], iteration=self.iteration)
			acc = cloned_task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last']

		row.append(acc)

		try:
			row.append(cloned_task.get_parameters()[self.hyperparameters_section+'/epochs'])
		except:
			try:
		...
		if(self.best_epoch):
			...
			if best_epoch_acc != None:
				acc = best_epoch_acc
				print('\n')
				print('Best epoch '+str(self.metric)+': ' + str(acc))
				print('\n')
	...
	def loss_objective(self, **data):
		...
		if(self.best_epoch):
			...
			if best_epoch_loss != None:
				...
				print('\n')
				print('Best epoch '+str(self.metric)+': ' + str(loss))
				print('\n')
...
# bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='base task', params=param,metric='loss', metric_sign = 'min',hyperparameters_section='General', objective_metric_title='epoch_loss', objective_metric_series='validation: epoch_loss',tuner_project_name='BO_tuning_3', tuner_task_name='hyperparameter tuning', optimizer='BOHB')
bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='base task', params=param,metric='accuracy', metric_sign = 'max',hyperparameters_section='General', objective_metric_title='epoch_accuracy', objective_metric_series='validation: epoch_accuracy',tuner_project_name='BO_tuning_3', tuner_task_name='Optuna hyperparameter tuning', optimizer='Optuna',time_limit_per_job=20.)
# bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='base task', params=param,metric='loss', metric_sign = 'min',hyperparameters_section='General', objective_metric_title='epoch_loss', objective_metric_series='validation: epoch_loss',tuner_project_name='BO_tuning_3', tuner_task_name='hyperparameter tuning', optimizer='BayesianOptimization', best_epoch=True)
# bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='pytorch base', params=param,metric='loss', metric_sign = 'min',hyperparameters_section='Args', objective_metric_title='test', objective_metric_series='loss',tuner_project_name='BO_tuning_3', tuner_task_name='pytorch hyperparameter tuning', optimizer='BOHB')
bayes.start()