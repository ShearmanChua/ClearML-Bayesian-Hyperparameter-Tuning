...
class BOtask:
	def __init__(self, base_task_project_name, base_task_name, params,metric, hyperparameters_section, objective_metric_title, objective_metric_series, tuner_project_name='hyperparameter_tuning', tuner_task_name='hyperparameter tuning', optimizer='BayesianOptimization', best_epoch=False, local = False,BO_n_iter=10, BO_init_points=2,local_metrics_titles=[],local_metrics_series=[],max_number_of_concurrent_tasks=1,execution_queue='default',time_limit_per_job=10., pool_period_min=0.2, total_max_jobs=10, min_iteration_per_job=3,max_iteration_per_job=6):
		...
		if(not local):
			...
			print('\n')
			print("USING LOCAL OPTIMIZATION METHOD!!!")
			print('\n')
		...
		self.local = local
		self.BO_n_iter = BO_n_iter
		self.BO_init_points = BO_init_points
		self.local_metrics_titles = local_metrics_titles
		self.local_metrics_series = local_metrics_series
		self.tuner_project_name = tuner_project_name
		self.tuner_task_name = tuner_task_name
		...
		self.local_tasks_names = []
		self.local_tasks_ids = []
	...
	def start(self):
		...
		if(self.optimizer=='Optuna' and optunaStrategy):
			...
			self.BOHB_tuner()
		elif(self.optimizer=='BayesianOptimization' and self.local):
			print('\n')
			print("ENTERING LOCAL BO_tuner method")
			self.BO_tuner_local()
	...
	def Optuna_tuner(self):
		...
		an_optimizer.set_time_limit(in_minutes=180.0)
		...
		self.df = self.df.sort_values(by=[self.metric], ascending=False)
	...
	def BO_tuner(self):
		...
		bayes_op.maximize(n_iter=self.BO_n_iter, init_points=self.BO_init_points)
		...
		for index, row in self.df.iterrows():
    			self.logger.report_scalar(title=self.objective_metric_title,series=self.objective_metric_series, value=row[self.metric], iteration=index)
	...
	def best_epoch_objective(self, cloned_task, row):
		print('\n')
		print('USING BEST EPOCH OBJECTIVE FUNCTION:\n')
		print(cloned_task.get_parameters())
		print('\n')

		try:
			epochs = cloned_task.get_reported_scalars()[self.objective_metric_title][self.objective_metric_series]['x']
			objectives = cloned_task.get_reported_scalars()[self.objective_metric_title][self.objective_metric_series]['y']
			objective = objectives[0]
			epoch = 0
			for i in range(1, len(objectives)):
				if(self.metric == 'loss'):
					...
					if(objectives[i]>objective):
						objective = objectives[i]
						epoch = i
			print("Best " + self.metric + ": " + str(objective))
			print("Obtained at " + str(epoch) + " epoch")

			row.append(objective)
			row.append(epoch)

			self.df_best.loc[self.iteration] = row

			df = pd.DataFrame(columns=self.columns)
			df.loc[0] = row

			if(not self.local):
				self.logger.report_table(title=cloned_task.id,series="Best " + self.objective_metric_series + ' DataFrame',iteration=self.iteration,table_plot=df)
	...
	def acc_objective(self, **data):
		...
		try:
			dic = cloned_task.get_parameters_as_dict()
			for key, value in dic.items():
				if ("epoch" in key) or ("epochs" in key):
					print(key)
					row.append(value)
				if isinstance(value, dict):
					for k,v in value.items():
						if ("epoch" in k) or ("epochs" in k):
							print(k)
							row.append(v)
						if isinstance(v, dict):
							for i,j in v.items():
								if ("epoch" in i) or ("epochs" in i):
									print(i)
									row.append(j)
		except:
			try:
				row.append(cloned_task.get_parameters()[self.hyperparameters_section+'/epoch'])
			except:
				print("Could not retrieve final epoch value")
				row.append('last')

		self.df.loc[self.iteration] = row

		if(self.best_epoch):
			best_epoch_acc = self.best_epoch_objective(cloned_task, row[:-2])

			if best_epoch_acc != None:
				acc = best_epoch_acc
				print('\n')
				print('Best epoch accuracy: ' + str(acc))
				print('\n')
	...
	def loss_objective(self, **data):
		...
		if(self.best_epoch):
			best_epoch_loss = self.best_epoch_objective(cloned_task, row[:-2])

			if best_epoch_loss != None:
				loss = best_epoch_loss
				print('\n')
				print('Best epoch loss: ' + str(loss))
				print('\n')
	...
	def BO_tuner_local(self):
    		
		print("This is BO_tuner_local function")
		print('\n')
    		
		if self.metric == 'accuracy':
			print("Optimizing model accuracy")
			objective = self.acc_objective_local
		elif self.metric == 'loss':
			print("Optimizing model loss")
			objective = self.loss_objective_local
		...
		parser = pipeline.AnnotatedDataPipeline.add_pipeline_args(parser)
		args = parser.parse_args()

		task = Task.init(project_name=self.tuner_project_name, task_name="Hyperparameter Tuning")
		...
		task.get_logger().report_table(title='tuning experiments',series='pandas DataFrame',iteration=0,table_plot=self.df)
		task.get_logger().report_table(title='tuning experiments best',series='best metrics at particular epoch DataFrame',iteration=0,table_plot=self.df_best)

		try:

			for index, row in self.df.iterrows():
				task.get_logger().report_scalar(title=self.objective_metric_title,series=self.objective_metric_series, value=row[self.metric], iteration=index)

			for name in range(0,len(self.local_tasks_names)):
					
				print('\n')
				print(self.local_tasks_names[name])
				print('\n')
				
				tuner_task = Task.get_task(project_name=self.tuner_project_name, task_name=self.local_tasks_names[name], task_id= self.local_tasks_ids[name])
				
				for i in range(0, len(self.local_metrics_titles)):
					
					print(self.local_metrics_titles[i])
			
					epochs = tuner_task.get_reported_scalars()[self.local_metrics_titles[i]][self.local_metrics_series[i]]['x']
					objectives = tuner_task.get_reported_scalars()[self.local_metrics_titles[i]][self.local_metrics_series[i]]['y']


					for epoch in range(0,len(epochs)):

						task.get_logger().report_scalar(title=self.local_metrics_titles[i] +" "+self.local_tasks_names[name],series=self.local_metrics_series[i], value=objectives[epoch], iteration=epochs[epoch])


		except:
			print('\n')
			print("Not able to log local tasks scalars")
			print('\n')

		try:
			task.upload_artifact('best_params', dict)
		except:
			print("Fail to upload best params dict")

	def acc_objective_local(self, **data):
		...
		task = Task.init(project_name=self.tuner_project_name, task_name="Tuning Experiment {}".format(json.dumps(data)))
		model_config_dict = task.connect_configuration(cfg,name='Model Training Parameters')
		pipeline_config_dict = task.connect_configuration(pipeline_cfg,name='Data Pipeline Parameters')

		task.set_parameters(vars(args))

		self.local_tasks_names.append("Tuning Experiment {}".format(json.dumps(data)))
		self.local_tasks_ids.append(task.id)
		...
		print('\n')
		print("PLEASE WAIT. RETRIEVING METRICS FROM EXPERIMENT....")
		print('\n')

		time.sleep(30)

		if(task.get_last_scalar_metrics()):
			print('\n')
			...
			print('\n')
			# self.logger.report_scalar(title='val_loss',series='loss', value=cloned_task.get_last_scalar_metrics()['evaluate']['loss']['last'], iteration=self.iteration)
			# task.get_logger().report_scalar(title='val_acc',series='accuracy', value=task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last'], iteration=self.iteration)
			acc = task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last']

		row.append(acc)

		try:
			dic = task.get_parameters_as_dict()
			for key, value in dic.items():
				if ("epoch" in key) or ("epochs" in key):
					print(key)
					row.append(value)
			...
			try:
				row.append(task.get_parameters()[self.hyperparameters_section+'/epoch'])
			except:
				print("Could not retrieve final epoch value")
				row.append('last')
		...
		if(self.best_epoch):
			best_epoch_acc = self.best_epoch_objective(task, row[:-2])
	...
	def loss_objective_local(self, **data):
		...
		if(task.get_last_scalar_metrics()):
			...
			print(task.get_parameters())
			print('\n')
			# task.get_logger().report_scalar(title='val_loss',series='loss', value=task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last'], iteration=self.iteration)
			# self.logger.report_scalar(title='val_acc',series='accuracy', value=cloned_task.get_last_scalar_metrics()['evaluate']['accuracy']['last'], iteration=self.iteration)
			loss = task.get_last_scalar_metrics()[self.objective_metric_title][self.objective_metric_series]['last']
		...
		if(self.best_epoch):
			best_epoch_loss = self.best_epoch_objective(task, row[:-2])
...
# bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='base task', params=param,metric='accuracy', hyperparameters_section='General', objective_metric_title='epoch_accuracy', objective_metric_series='validation: epoch_accuracy',tuner_project_name='BO_tuning_3', tuner_task_name='Optuna hyperparameter tuning', optimizer='Optuna',time_limit_per_job=20.)
# bayes = BOtask(base_task_project_name='BO_tuning_3', base_task_name='base task', params=param,metric='loss', hyperparameters_section='General', objective_metric_title='epoch_loss', objective_metric_series='validation: epoch_loss',tuner_project_name='BO_tuning_3', tuner_task_name='hyperparameter tuning', optimizer='BayesianOptimization', best_epoch=True)
